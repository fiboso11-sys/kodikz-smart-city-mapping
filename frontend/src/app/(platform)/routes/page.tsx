  ...
  return (
    <div className="flex h-[100dvh] flex-col gap-3 overflow-hidden p-3 pb-[max(0.75rem,env(safe-area-inset-bottom))] md:gap-4 md:p-4">
      <header className="shrink-0">
        <h1 className="text-xl font-semibold text-white md:text-2xl">Route Management</h1>
        <p className="text-xs text-slate-500 md:text-sm">Upload GeoJSON · assign vehicles & permits</p>
      </header>
      <div className="flex min-h-0 flex-1 flex-col gap-3 md:flex-row md:gap-4">
        <div className="glass-panel order-1 shrink-0 overflow-hidden rounded-xl p-1 md:order-2 md:min-h-0 md:flex-1">
          <LiveMap />
        </div>
        <div className="order-2 max-h-[40dvh] w-full space-y-4 overflow-y-auto md:order-1 md:max-h-none md:w-96">
